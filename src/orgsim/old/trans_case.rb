...
module Simulator
  module Orgsim
    class CaseField
      ...
      def applyHook
        eval "@action.fldDefs.#{name} = @packHook.pack(@action.fldDefs.#{name})" if @packHook
      end

      def genValue
        value = @generator.value
        # can't use @df.value = @generator.value, cause this will not evaluate the parent value
        eval "@action.fldDefs.#{name} = value"
#        @df.value = value
      end

      def setValue(val)
        eval "@action.fldDefs.#{name} = val"
      end
...
#        define_attr(fld) if fld.generator.kind_of? ValueTypeUserInput
      ...
      def unpackPrevMessage
        if @prevIncomingFlddef
          ...
          @prevIncomingFlddef.unpack(TrxnLog.prevIncoming)
        end

        if @prevOutgoingFlddef
          @prevOutgoingFlddef.clear
          @prevOutgoingFlddef.unpack(TrxnLog.prevOutgoing)
        end
      end
