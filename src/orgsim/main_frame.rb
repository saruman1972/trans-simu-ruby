...
module Simulator
    module Orgsim
        ...
        class TrxnLogList < Wx::ListCtrl
            def initialize(parent)
                super(parent, -1, :style => Wx::LC_REPORT | Wx::LC_HRULES | Wx::LC_VRULES)#, :size => Wx::Size.new(500, 100))
                insert_column(0, "Severity")
                insert_column(1, "Timestamp")
                insert_column(2, "Case Name")
                insert_column(3, "Message Name")

                loadTrxns
            end

            def loadTrxns
                rows = TrxnLog.loadTrxnLogs
                rows.each_with_index {|row,idx|
                    insert_item(idx, row[0])
                    set_item(idx, 1, row[1])
                }
            end
        end
        ...
        class CardList < Wx::ListCtrl
            def initialize(parent)
                ...
                insert_column(1, "Cardno")
                insert_column(2, "Expiry Date")
                insert_column(3, "Service Code")
                insert_column(4, "CVV")
                insert_column(5, "CVV2")
                insert_column(6, "PIN")
                insert_column(7, "Track2")
                insert_column(8, "Track3")
                insert_column(9, "ID Type")
                insert_column(10, "ID No")
                insert_column(11, "Trxn Amount")
                insert_column(12, "IC Seq")
                insert_column(13, "IC AIP")
                insert_column(14, "IC ATC")
                insert_column(15, "IC TVR")
                insert_column(16, "IC IAD")
            end
        end

        class AcquirerList < Wx::ListCtrl
            def initialize(parent)
                super(parent, -1, :style => Wx::LC_REPORT | Wx::LC_HRULES | Wx::LC_VRULES)#, :size => Wx::Size.new(500, 100))
                insert_column(0, "Name")
                insert_column(1, "Merchant ID")
                insert_column(2, "Terminal ID")
                insert_column(3, "MCC")
                insert_column(4, "Trxn Curr CD")
                insert_column(5, "ACQ INST CD")
                insert_column(6, "FWD INST CD")
                insert_column(7, "IC Terminal Type")
                insert_column(8, "IC IFD")
                insert_column(9, "IC DF")
                insert_column(10, "IC TAV")
                insert_column(11, "IC TSC")
            end
        end
        ...
        class MainFrame < Wx::Frame
            ...
            def initialize(parent, id, title, pos=Wx::DEFAULT_POSITION, size=Wx::DEFAULT_SIZE, style=Wx::DEFAULT_FRAME_STYLE)
                ...
                notebook = Wx::Notebook.new(@splitter)
                @logList = TrxnLogList.new(notebook)
                notebook.add_page(@logList, "Trasaction Logs", true)
                @cardList = CardList.new(notebook)
                notebook.add_page(@cardList, "Cards")
                @acquirerList = AcquirerList.new(notebook)
                notebook.add_page(@acquirerList, "Acquirers")
                @splitter.split_horizontally(@top_splitter, notebook, 300)
