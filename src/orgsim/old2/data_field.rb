    ...
    class LengthField < DataField
      ...
      def repackField
        len = if @term
                 ...
                 @parent.offset + @parent.packedValueWithLength.length
      ...
      def repack
        @fwrdRfrs.each do |fld|
          left = if fld.offset==0 
                   ...
                   @packedValue[0..fld.offset-1]
                 end
          right = if fld.offset+fld.packedValueWithLength.length > @packedValue.length
                    ''
                  else
                    @packedValue[fld.offset+fld.packedValueWithLength.length..-1]
                  end
          @packedValue = left + fld.repackField + right
        end
        @value = @packedValue
        @packedValueWithLength = @lenStr + @packedValue
      end
