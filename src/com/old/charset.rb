...
#0xF0
    ...
    def atoe(asciiStr)
      asciiStr.split(//).collect {|c| @@a2e[c.ord]} .pack("C*")
    end

    def etoa(ebcdicStr)
      ebcdicStr.split(//).collect {|c| @@e2a[c.ord]} .pack("C*")
    end
  ...
  @@asciiCodec = Ascii.new
  @@ebcdicCodec = Ebcdic.new

  def Charset.getAsciiCodec
    @@asciiCodec
  end

  def Charset.getEbcdicCodec
    @@ebcdicCodec
  end
end
