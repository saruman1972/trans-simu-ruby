...
class InplaceEditListCtrl < Wx::ListCtrl
    ...
    def hit_test_ex(pos)
        row = get_top_item
        item_rect = get_item_rect(row)
        new_pos = Wx::Point.new(pos.x, pos.y+item_rect.y)
        bottom = row + get_count_per_page
        bottom = get_item_count if bottom > get_item_count
        row.upto(bottom-1) {|index|
            item_rect = get_item_rect(index)
            if rect.contains(pos)
                startx = item_rect.x
                get_column_count.times {|col|
                    col_rect = Wx::Rect.new(startx, item_rect.y, get_column_width(col), item_rect.height)
                    return index,col if col_rect.contains(new_pos)
                    startx += get_column_width(col)
...
#            InplaceChoice.new(row, col, self, -1, rect.top_left, rect.get_size, attr.choices, get_item(row,col).get_text)
            InplaceChoice.new(row, col, get_parent, -1, Wx::Point.new(100,100), Wx::Size.new(100,20), attr.choices, get_item(row,col).get_text)
        end
    end
